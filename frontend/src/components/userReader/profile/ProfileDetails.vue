...
export default {
  ...
  methods: {
    ...
    isValidPatronymic() {
      return /^[А-ЩЬЮЯҐЄІЇ][а-щьюяґєії']{1,20}$/.test(
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            axios
              ...
              .catch((error) => {
                if (error.response) {
                  console.warn("Server responded with:", error.response.data);

                  const errorMessage = error.response.data.Message;

                  let userMessage =
                    "Не вдалося оновити профіль. Спробуйте пізніше!";
                  if (errorMessage === "username is already taken!") {
                    userMessage = "Логін вже зайнятий!";
                  } else if (errorMessage === "email is already taken!") {
                    userMessage = "Електронна пошта вже зайнята!";
                  } else if (
                    errorMessage === "phone number is already taken!"
                  ) {
                    userMessage = "Номер телефону вже зайнятий!";
                  }
                  ...
                  Swal.fire({
                    icon: "error",
                    title: "Помилка!",
                    text: "Сервер не відповідає. Перевірте з'єднання!",
                  });
                } else {
                  ...
                  Swal.fire({
                    icon: "error",
                    title: "Помилка!",
                    text: "Невідома помилка! Перевірте консоль розробника.",
                  });
                }
              })
              .catch(() => {});
